...
from .models import RequestContext, HarborRegistryConfig
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Harbor Helper locally for testing."
    )
    parser.add_argument(
        "query", nargs="?", help="The raw user request (e.g., 'create project test')"
    )
    parser.add_argument(
        "--jira", help="A pre-existing JIRA ticket key (e.g., PRODENG-123)"
    )
    parser.add_argument(
        "--user", default="U123456", help="The simulated user ID (Slack ID format)"
    )
    parser.add_argument("--channel", default="C123456", help="The simulated channel ID")
    parser.add_argument(
        "--approver", default="U654321", help="The simulated approver ID"
    )
    parser.add_argument(
        "--non-interactive", action="store_true", help="Auto-approve without user input"
    )
    parser.add_argument(
        "--real-api",
        action="store_true",
        help="Use real API clients (requires ENV vars)",
    )
    parser.add_argument(
        "--ollama-url",
    ...
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Display LLM prompt, response, and side-effects",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--registry",
        action="append",
        help="Harbor registry config (nickname:url:user:pass)",
    )
    parser.add_argument(
        "--existing-project", action="append", help="Simulate existing project in mocks"
    )
    parser.add_argument(
        "--existing-robot", action="append", help="Simulate existing robot in mocks"
    )
    ...
    if args.real_api:
        ...
        harbor = MockHarborClient(
            existing_projects=args.existing_project, existing_robots=args.existing_robot
        )
        ...
        while True:
            try:
                ...
                if not query:
                    continue
                jira_key = (
                    input("[REPL] JIRA Ticket (optional) > ").strip() or args.jira
                )
